...
|-------------------------|------------------------------------------|----------------|--------------------------------------------------|
| WP-L1 acceptance criterion: "adding a new extractor requires touching only the registry + a new extractor file" (`DOCS/audit/03-Multi-Language-Graph-Plan.md` §3 WP-L1, not a Core Principle but a stated design invariant this feature must either honor or explicitly break) | Add `INTERFACE` to `GraphAssembler._attach_defines`'s `definition_types`, `DOCUMENTABLE_TYPES`, and the CLASS-only filter in `_resolve_inheritance` (3 set/filter edits, no new resolution algorithm) — **plus, discovered during implementation, `symbol_table.py`'s indexed-type set** (`_PRIORITY` and `build_symbol_table`'s filter), since `_resolve_base`/`_resolve_via_binding` for an `implements`/`extends` target resolve through the symbol table, not `graph.entities` directly; without this fix `implements SomeInterface` silently fell through to `EXTERNAL_SYMBOL` even when `SomeInterface` was defined and imported in-repo (caught by this feature's own end-to-end smoke test, not by inspection alone) | TypeScript's `interface` has no Python equivalent; without all four touches, `interface`/`extends`/`implements` on interfaces would silently produce no DEFINES/INHERITS/DOCUMENTS edges — an acceptance-criterion failure (spec FR-008, User Story 3 scenarios 3-4), not a cosmetic gap | Emitting interfaces with `kind="CLASS"` plus `metadata.is_interface=True` was considered and rejected: it would make `CLASS` do double duty as an identity concept for two different TS/JS language constructs, which ADR-031 canonical-identity discipline treats as a modeling decision, not a query hack — a real `artifact_type` value is the honest representation, and the plan doc's own target architecture table already names `INTERFACE` as a first-class kind (§2) |
