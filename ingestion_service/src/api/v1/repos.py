...
async def delete_repo(repo_id: str):
    ...
    Order matters and is not arbitrary (see issue #158):
      1. Enumerate every historical ingestion_id for repo_id *first* —
         document_nodes.repo_id is the only place this mapping exists,
         so it must be captured before step 3 deletes those rows.
    ...
    try:
        with SessionLocal() as session:
            nodes_deleted = CodebaseGraphPersistence(session=session).delete_repo_nodes(
                repo_id
            )
    except Exception as e:
