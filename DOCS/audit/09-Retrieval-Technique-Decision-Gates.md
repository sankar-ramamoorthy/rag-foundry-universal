...
| **Reranking** | deferred / NO-GO | NO-GO per the WP-Q0 reranker decision gate (issue #49). See [[04-Scalability-Plan#WP-S8 — Retrieval quality/perf at scale\|WP-S8]] and [[08-RAG-Quality-Evaluation-Methodology#4 · Reranker decision gate\|the methodology's gate]] for the mechanics — not restated here. 2026-09-15: issue #149 found a TradeForge candidate (`handleRequestFundamentals`, true raw-cosine rank 75) that looked like a reranker case, but the root cause turned out to be issue #150 (HNSW post-filter under-recall silently capping filtered searches at ~28 rows regardless of requested `k`, fixed via pgvector iterative scans — `DOCS/test_results/2026-09-15-hnsw-iterative-scan-issue-150.md`) — the chunk wasn't merely ranked low, it usually wasn't being examined at all. Reconsider only against the *post-#150* failure shape. The frozen set was re-run post-#150 the same day, per this decision (`DOCS/notes/20260915-reranker-built-ahead-of-evaluation-gate-decision.md`, WP-S8/#152 built the on/off infra first): `DOCS/test_results/2026-09-15-wp-s8-rerank-evidence-survival-run.md` found the run cannot supply rank-8-20 evidence either way — 6 of 7 `rag-foundry-universal` candidates had zero real-source chunks in the candidate pool at all (issue #141, not a ranking problem), and reranking actively dropped the one candidate whose real chunk had survived. The one uncontaminated candidate (TradeForge) is a single positive data point, not sufficient evidence. Still deferred / NO-GO. | A non-trivial set of failures has the correct evidence retrieved but ranked below the context cutoff, especially roughly ranks 8–20. WP-Q0 had 0/10 such cases. Re-run against an uncontaminated corpus (or after #141 is fixed) with at least 2 independent questions before treating any low-rank case as reranker evidence. |
...
| **Context deduplication** | validated (implemented) | [Issue #65](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/65) (near-duplicate module/sole-child chunk crowding, measured at a mean of 4 duplicate candidate slots per code question in WP-Q0) is fixed in [#73](https://github.com/sankar-ramamoorthy/rag-foundry-universal/pull/73). The "where should it happen" question is answered: **retrieval time**, inside `hybrid_retrieve()` — not ingestion, because ADR-039 §4 / ADR-040 §6.2 require embedding every MODULE artifact and changing that would need amending an accepted ADR plus re-ingesting every existing repo; retrieval-time dedup works immediately on already-ingested data instead. | Revisit if duplicate crowding reappears in a form the containment-ratio heuristic (`dedupe_near_identical_chunks`, ≥85% shorter-in-longer text containment within the same file) doesn't catch — e.g. near-duplicates that aren't a strict container/child text relationship. |
...
| **Doc-type-aware seed ranking** (prefer implementation `document_nodes.doc_type` over documentation-about-implementation within a near-tie similarity-score band) | investigate | [Issue #141](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/141) found, with direct DB verification, that a self-ingested Markdown eval doc's near-verbatim question text outranked the real `base.py` implementation it discussed — confirmed the real answer was excluded from the seed search's own top-k-bounded result set entirely, not merely outranked within it (`DOCS/test_results/2026-09-14-doc-type-tie-break-issue-141.md`). A fix implemented in [issue #142](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/142) (`DOC_TYPE_TIE_BREAK_ENABLED`, default off) is unit-tested (7 tests, including the exact promotion shape and a control case). A 2026-09-15 live attempt to re-verify against the redeployed, re-ingested branch found the original repro query (and several other frozen-set candidates) no longer exhibits the failure at all, flag on or off — the corpus has drifted enough since 2026-09-07 (edited source files, re-chunking) that the specific near-tie this issue documented no longer occurs for these exact queries. Live re-verification of the mechanism itself therefore remains open, not failed. **Superseded same day:** a later 2026-09-15 run (`DOCS/test_results/2026-09-15-wp-s8-rerank-evidence-survival-run.md`) reproduced the identical query's failure again, at full severity (zero real-source chunks in final context, not a near-tie), flag off. Both observations are from live checks against a corpus being re-ingested/edited during the same day — treat same-day "no longer occurs" claims as unpinned and unreliable, not as resolution. The failure is confirmed still-open as of the later run. | A live query that reproduces a genuine current-corpus near-tie (an implementation chunk within `DOC_TYPE_TIE_BREAK_EPSILON` of a documentation chunk, on the wrong side of the `top_k` cutoff) confirms the real implementation now survives to final context without regressing a documentation-seeking control question — promotes to `validated` and the flag's default flips to `True` (Stage B) only then, per this repo's WP-L5 rollout-flag precedent. |
...
- It is not a bug tracker. Issue #64 (the code-query seed filter's
  `doc_type` match never firing) was a plain correctness defect surfaced
  during WP-Q0, not an external technique under evaluation — it went
  through the normal bug-fix workflow (fixed in PR #72) and was never in
  scope for this table. Issue #65 appears above because it backed a
  genuine technique-level question (where should deduplication happen,
  now answered — see the row above), not because every WP-Q0 finding
  belongs here.
