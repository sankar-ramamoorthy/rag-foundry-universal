...
- **[Issue #83](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/83) (`WP-L2`) done**: the first new language lands. A tree-sitter runtime + TypeScript/JavaScript extractor (`ingestion_service/src/core/extractors/treesitter/`) produces MODULE/CLASS/INTERFACE/FUNCTION/METHOD nodes, IMPORTS (ESM + `require`), CALL sites, and EXTENDS/IMPLEMENTS edges, with a `CompositeModuleConvention` so one repo can mix Python and TS/JS in a single ingestion run. See `specs/002-typescript-js-extractor/` and [[03-Multi-Language-Graph-Plan#WP-L2]].
- **[Issue #85](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/85) (`WP-L6a`) done**: the retrieval-filter half of `WP-L6`, pulled forward ahead of `WP-L3`/`WP-L4` specifically to serve as the measurement apparatus validating `WP-L2` against a real mixed-language repo. `language` is derived centrally in `GraphAssembler` from file suffix and promoted to a typed, indexed `vector_chunks.language` column; `/v1/rag` accepts an optional `language` param that filters seeds (and survives the existing fallback relaxation the same way `repo_id` already did). The Gradio-dropdown half of `WP-L6` remains open.
- **What's next in Phase 3:** `WP-L3` (Rust) or `WP-L4` (Java) — neither started. See [[03-Multi-Language-Graph-Plan]].
- **RAG retrieval-quality bug found, fixed, and live-verified — [issue #89](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/89), closed via PR #90.** Independent of the WP-Q0/reranker track above: live diagnosis (`DOCS/test_results/2026-09-03-rag-retrieval-quality-linux-tailscale-baseline.md`, sections 13-16) found graph expansion can correctly discover authoritative implementation evidence that then gets discarded by *authority-blind* ranking at the `MAX_EXPANDED_DOCS` cap — a CALL-derived, less-relevant node could outrank a true DEFINES child of an already-matched seed purely on canonical-ID alphabetical order. Fixed with relation-type-aware expansion ranking (`rag_orchestrator/src/retrieval/traversal_selector.py`): ranking now keys on which traversal strategy discovered a node before falling back to seed-hit count. **Confirmed on live, freshly-ingested data**, not just synthetic fixtures: a target's rank moved from 24 to 12 and it started reaching final context that it previously didn't, with zero regressions across the rest of the live baseline questions (103/103 unit tests also pass). A live-eval side-finding worth carrying forward: this repo's own `DOCS/test_results/*.md` eval docs are now part of its ingested corpus and get retrieved as sources for the very questions they document the answers to — invalidating that fixed question set for future clean answer-quality regrades against this repo (retrieval-level rank/survival measurements are unaffected).
- **A distinct, still-open limitation — [issue #91](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/91), open.** Same live comparison found a second target whose rank improved substantially under the #89 fix (137 → 88) but still didn't clear the cap — a *same-relation-type candidate overload* case (many equally-authoritative DEFINES siblings competing for one seed's cap slots), which relation-type-aware ranking alone can't resolve. Deliberately not folded into #89's fix or closed by it; needs its own characterization pass before an intervention is chosen, per the same evidence-first discipline #89 used.
- **Exploratory findings preserved, not yet actioned:** `DOCS/notes/20260906-repository-understanding-trace-and-structural-artifacts.md` records design/evaluation observations from hands-on testing across five ingested repos (this one plus TradeForge, TradeForge-KnowledgeBase, TradeForge-ResearchCockpit, py-coding-agent) — repository-identity grounding failures, a cross-repo confirmed case of a doc file invisible to both vector and graph retrieval, and a "retells rather than derives" limitation on behavioral-trace questions. Explicitly marked as future input, not roadmap items.
- **Operational decision recorded:** `DOCS/notes/20260906-nightly-repo-ingestion-decision.md` — the Linux/Tailscale instance is treated as prod and will be re-ingested nightly (repo query `id` confirmed stable across re-ingestions); automation mechanism not yet chosen.
