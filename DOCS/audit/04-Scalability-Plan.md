...
> [!warning] Reranker sub-task: NO-GO as a *default*, deferred (2026-08-27); implemented flag-gated, off by default (2026-09-15)
> The WP-Q0 RAG-quality baseline (issue #49, `DOCS/test_results/2026-08-27-wp-q0-rag-quality-baseline.md`) ran the reranker decision gate below against a 10-question corpus and returned an explicit **NO-GO**: 0/10 failures classified `rank-8-to-20` (the only pattern a reranker addresses); the one failure was already top-3 and was independently confirmed via a clean-context test to be a prompting/context-assembly problem. **2026-09-15: built anyway, deliberately ahead of a fresh evaluation** ([issue #152](https://github.com/sankar-ramamoorthy/rag-foundry-universal/issues/152), rationale in `DOCS/notes/20260915-reranker-built-ahead-of-evaluation-gate-decision.md`) — `RERANK_ENABLED` defaults to `False`, so this is infrastructure for running the A/B comparison the gate itself asks for, not a bypass of it. The gate is unresolved, not satisfied: still pending a real on/off evaluation, now also after issue #150's HNSW recall fix changed the underlying retrieval picture.
...
- ~~Add a reranker stage (flag-gated): cross-encoder over top-50 → top-10 (runs in `rag_orchestrator`; model via [[06-LLM-Provider-LiteLLM-Plan|LiteLLM]] or local `bge-reranker`).~~ **Implemented 2026-09-15** (issue #152): a pointwise `sentence-transformers` `CrossEncoder` (`cross-encoder/ms-marco-MiniLM-L-6-v2` default, swappable via `RERANK_MODEL`) over the already-assembled candidate pool in both `run_rag()` and `run_simple_rag()`, `RERANK_ENABLED` off by default, per-request override via `RAGQuery.rerank`/`SimpleRAGQuery.rerank`. Built ahead of the gate below by deliberate decision — see the callout above and `DOCS/notes/20260915-reranker-built-ahead-of-evaluation-gate-decision.md`. **Gate still applies to flipping the default:** only flip `RERANK_ENABLED`'s default after [[08-RAG-Quality-Evaluation-Methodology]] shows correct chunks landing at rank 8–20, not absent or already top-3.
...
- [ ] Reranker flag on/off compared on a small eval set; results recorded in `DOCS/test_results/` — the flag now exists (issue #152) to actually run this comparison; the 2026-08-27 eval predates both the reranker build and issue #150's recall fix, so re-run rather than reuse that recorded NO-GO
