...
- Issue #41: `ingestion_service`/`llm_service`/`rag_orchestrator` each have their own independent `pyproject.toml`/`uv.lock`, but `CMD` is `uv run --directory /app/<service> ...`, which re-resolves that service's own env from scratch at **container start** rather than using anything baked into the image at build time — a multi-GB re-download on every recreate (mitigated but not fixed by #55/#57, which only made the CUDA-vs-CPU-only torch wheel choice correct, not the re-resolution itself). Fix: `RUN uv sync` for each service's own project directory at build time (not the unrelated root project, which is what the Dockerfiles do today), and drop the runtime resync (`uv run --no-sync` or invoke the baked venv's binary directly).
